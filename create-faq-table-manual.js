...
async function createSchema() {
  try {
    console.log('Creating Peace schema...');
    
    // First, let's try to create the schema
    const { data: schemaResult, error: schemaError } = await supabase.rpc('pg_create_schema', {
      schema_name: 'Peace'
    });
    
    if (schemaError && !schemaError.message.includes('already exists')) {
      console.log('Schema creation error (might be expected):', schemaError);
    } else {
      console.log('Peace schema created or already exists');
    }
    ...
    if (!checkError) {
      console.log('Table Hindu_faqs already exists! Checking record count...');
      
      const { count } = await supabase
        .from('Hindu_faqs')
        .select('*', { count: 'exact', head: true });
        
      console.log(`Hindu_faqs table contains ${count} records`);
      ...
      if (count > 0) {
        ...
        console.log('\n✅ FAQ table is already properly set up with sample data!');
        return;
      }
    }
    
    console.log('Table does not exist or is empty. Creating manually...');
    
    // Since we can't execute raw SQL, let's create records manually using the client
    const faqData = [
      {
    ...
    if (insertError) {
      console.error('Error inserting FAQ data:', insertError);
      return;
    }
    ...
    console.log(`\n✅ FAQ table created and populated successfully with ${count} records!`);
    ...
    console.error('Failed to create schema and table:', error);
  }
}
